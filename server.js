...
Server.prototype.init = function init() {
  ...
  this.httpServer.use(express.static("mapMaker"));
};

Server.prototype.initRoutes = function initRoutes() {
  this.httpServer.get("/", (req, res) => res.sendFile(INDEX));
  this.httpServer.get("/mapMaker", (req, res) =>
    res.sendFile(path.join(__dirname, "mapMaker/mapMaker.html"))
  );
};
