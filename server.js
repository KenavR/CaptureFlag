...
function Player(ws) {
	...
	this.id = Math.random();
	this.ws = ws;
	this.color = 'hsl(' + 240 + ',' + Math.floor(Math.random()*60+40) + '%,' + Math.floor(Math.random()*40+20) + '%)';
}
...
Player.prototype.update = function() {
	...
	if (this.velX<8 && this.velX>-8) {
		this.move[3] ? this.velX-=.5 : 0;
		this.move[1] ? this.velX+=.5 : 0;
	}
	if (this.velY<8 && this.velY>-8) {
		this.move[0] ? this.velY-=.5 : 0;
		this.move[2] ? this.velY+=.5 : 0;
	}
...
Player.prototype.collision = function() {
	try {
		...
		for (var i=0; i<collideGrids.length; i++) {
			if (grids[collideGrids[i]].type == 1) {
				var a = circRectCollision(this, grids[collideGrids[i]]);

				if (a == 'X') {
					var xAxisDist = this.x-grids[collideGrids[i]].x;
					var xAxisDist2 = grids[collideGrids[i]].x+100-this.x;
					xAxisDist < xAxisDist2 ? this.velX = -this.velX-1 : this.velX = -this.velX+1;
					break;
				}
				else if (a == 'Y') {
					var yAxisDist = this.y-grids[collideGrids[i]].y;
					var yAxisDist2 = grids[collideGrids[i]].y+100-this.y;
					yAxisDist < yAxisDist2 ? this.velY = -this.velY-1 : this.velY = -this.velY+1;
					break;
				}
				else if (a == true) {
					var xAxisDist = this.x-grids[collideGrids[i]].x;
		...
		for (var i=0; i<players.length; i++) {
			if (players[i].id != this.id) {
				...
				distance < 80 ? c2c(this, players[i]) : 0;
			}
		}
	}
	catch (e) {
